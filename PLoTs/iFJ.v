...
Section Proofs.
  ...
  Lemma Ty_eq_dec : forall (S T : Ty), {S = T} + {S <> T}.
    destruct S; destruct T; try (right; congruence).
    ...
    destruct i; destruct i0; destruct f; destruct f0;
      try destruct (eq_nat_dec n n0); subst; first [left; auto; fail | right; congruence].
  Qed.
  ...
  Fixpoint Subtype_Update_list_id' gamma S T (sub_S_T : subtype gamma S T) :=
    match sub_S_T in (subtype gamma S T) return 
      ...
      | I_subtype_Wrap gamma S' T' sub_S_T' => I_Weaken_Subtype_update_list_id'
        _ _ _ I_Ty_Wrap _ _ _ _ _ _ _ _ CT isub_build_te implements
        cFJ subtype I_subtype_Wrap Update gamma S' T' sub_S_T'
    end.
  ...
  Qed.
    ...
    Lemma FJ_Cast_discriminate : forall (e : FJ_E _ _ _ _ ty_ext E m_call_ext)
      (S : cFJ.FJ_Ty _ ty_ext) (e' : E), cFJ_E e <> Cast_E (Cast.cast nat ty_ext E S e').
      congruence.
    Qed.
